...
class ProposalOnlyPathModel(ContextAwareModel):
    def invoke_structured(
        self,
        *,
        system: str,
        ...
        if schema is ImpactAnalysis:
            return ImpactAnalysis(
                needs_documentation_update=True,
                summary="A guide update is needed.",
                candidate_files=[],
                findings=[
                    Finding(
                        category="behaviour_change",
                        evidence="The diff changes user-visible behaviour.",
                        confidence=0.9,
                    )
                ],
                uncertainty=None,
        ...
        if schema is DocumentationProposal:
            ...
            return DocumentationProposal(
                action="update",
                summary="Update the guide.",
                edits=[
                    {
                        "path": "docs/guide.md",
                        "operation": "append_section",
                        "rationale": "The guide covers this workflow.",
                        "proposed_markdown": "## New workflow\n\nUse the new flow.",
                    }
                ],
                reviewer_notes=[],
            )
...
def test_proposed_edit_paths_are_read_before_patch_generation(tmp_path) -> None:
    docs = tmp_path / "docs"
    docs.mkdir()
    (docs / "guide.md").write_text("# Guide\n", encoding="utf-8")
    model = ProposalOnlyPathModel()

    result = run_agent(
        diff="+The workflow behaviour changed.",
        settings=Settings(
            openai_api_key=None,
            allowed_doc_dirs="docs,README.md",
            repository_root=str(tmp_path),
    ...
    assert result.status == "ok"
    assert "--- a/docs/guide.md" in (result.proposal.edits[0].unified_diff or "")
    assert " # Guide" in (result.proposal.edits[0].unified_diff or "")
    assert "## New workflow" in (result.proposal.edits[0].unified_diff or "")
...
class InvalidReplaceModel(ContextAwareModel):
    def invoke_structured(
        self,
        *,
        system: str,
        ...
        if schema is DocumentationProposal:
            return DocumentationProposal(
                action="update",
                summary="Replace an ambiguous section.",
                edits=[
                    {
                        "path": "docs/api.md",
...
def test_invalid_edit_operation_blocks(tmp_path) -> None:
    docs = tmp_path / "docs"
    docs.mkdir()
    ...
    result = run_agent(
        ...
        model=InvalidReplaceModel(),
    )

    assert result.status == "blocked"
    assert result.proposal.action == "human_review"
    assert "invalid_edit_operation:docs/api.md:heading_not_unique" in result.safety_flags
